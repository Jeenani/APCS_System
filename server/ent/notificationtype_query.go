...
func (_q *NotificationTypeQuery) QueryNotifications() *NotificationQuery {
...
func (_q *NotificationTypeQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *NotificationTypeQuery) FirstIDX(ctx context.Context) int {
...
func (_q *NotificationTypeQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *NotificationTypeQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *NotificationTypeQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *NotificationTypeQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *NotificationTypeQuery) WithNotifications(opts ...func(*NotificationQuery)) *NotificationTypeQuery {
	query := (&NotificationClient{config: _q.config}).Query()
...
//		GroupBy(notificationtype.FieldCode).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.NotificationType.Query().
//		Select(notificationtype.FieldCode).
//		Scan(ctx, &v)
...
func (_q *NotificationTypeQuery) loadNotifications(ctx context.Context, query *NotificationQuery, nodes []*NotificationType, init func(*NotificationType), assign func(*NotificationType, *Notification)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*NotificationType)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(notification.FieldNotificationTypeID)
	}
	query.Where(predicate.Notification(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(notificationtype.NotificationsColumn), fks...))
	}))
...
func (_q *NotificationTypeQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(notificationtype.Table, notificationtype.Columns, sqlgraph.NewFieldSpec(notificationtype.FieldID, field.TypeInt))
	_spec.From = _q.sql
