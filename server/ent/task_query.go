...
// QueryPriority chains the current query on the "priority" edge.
func (_q *TaskQuery) QueryPriority() *PriorityQuery {
	query := (&PriorityClient{config: _q.config}).Query()
...
func (_q *TaskQuery) QueryStatus() *TaskStatusQuery {
	query := (&TaskStatusClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TaskQuery) QueryCategory() *TaskCategoryQuery {
	query := (&TaskCategoryClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TaskQuery) QueryHistories() *TaskHistoryQuery {
...
func (_q *TaskQuery) QueryNotifications() *NotificationQuery {
...
func (_q *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withPriority:      _q.withPriority.Clone(),
...
// WithPriority tells the query-builder to eager-load the nodes that are connected to
// the "priority" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TaskQuery) WithPriority(opts ...func(*PriorityQuery)) *TaskQuery {
	query := (&PriorityClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPriority = query
	return _q
}
...
func (_q *TaskQuery) WithStatus(opts ...func(*TaskStatusQuery)) *TaskQuery {
	query := (&TaskStatusClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithCategory(opts ...func(*TaskCategoryQuery)) *TaskQuery {
	query := (&TaskCategoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithHistories(opts ...func(*TaskHistoryQuery)) *TaskQuery {
	query := (&TaskHistoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithNotifications(opts ...func(*NotificationQuery)) *TaskQuery {
	query := (&NotificationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withPriority; query != nil {
		if err := _q.loadPriority(ctx, query, nodes, nil,
			func(n *Task, e *Priority) { n.Edges.Priority = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withStatus; query != nil {
		if err := _q.loadStatus(ctx, query, nodes, nil,
			func(n *Task, e *TaskStatus) { n.Edges.Status = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCategory; query != nil {
		if err := _q.loadCategory(ctx, query, nodes, nil,
			func(n *Task, e *TaskCategory) { n.Edges.Category = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withHistories; query != nil {
		if err := _q.loadHistories(ctx, query, nodes,
			func(n *Task) { n.Edges.Histories = []*TaskHistory{} },
			func(n *Task, e *TaskHistory) { n.Edges.Histories = append(n.Edges.Histories, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *TaskQuery) loadPriority(ctx context.Context, query *PriorityQuery, nodes []*Task, init func(*Task), assign func(*Task, *Priority)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		fk := nodes[i].PriorityID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "priority_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TaskQuery) loadStatus(ctx context.Context, query *TaskStatusQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskStatus)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		fk := nodes[i].StatusID
	...
	query.Where(taskstatus.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
...
func (_q *TaskQuery) loadCategory(ctx context.Context, query *TaskCategoryQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskCategory)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
...
func (_q *TaskQuery) loadHistories(ctx context.Context, query *TaskHistoryQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskHistory)) error {
	...
	query.Where(predicate.TaskHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.HistoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TaskQuery) loadNotifications(ctx context.Context, query *NotificationQuery, nodes []*Task, init func(*Task), assign func(*Task, *Notification)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Task)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(notification.FieldTaskID)
	}
	query.Where(predicate.Notification(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.NotificationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TaskQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withPriority != nil {
			_spec.Node.AddColumnOnce(task.FieldPriorityID)
		}
		if _q.withStatus != nil {
			_spec.Node.AddColumnOnce(task.FieldStatusID)
		}
		if _q.withCategory != nil {
			_spec.Node.AddColumnOnce(task.FieldCategoryID)
		}
