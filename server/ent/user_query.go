...
func (_q *UserQuery) QueryNotificationSetting() *NotificationSettingQuery {
...
func (_q *UserQuery) QueryTaskHistories() *TaskHistoryQuery {
...
func (_q *UserQuery) QueryExportLogs() *ExportLogQuery {
...
func (_q *UserQuery) WithNotificationSetting(opts ...func(*NotificationSettingQuery)) *UserQuery {
	query := (&NotificationSettingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withNotificationSetting = query
	return _q
}
...
func (_q *UserQuery) WithTaskHistories(opts ...func(*TaskHistoryQuery)) *UserQuery {
	query := (&TaskHistoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithExportLogs(opts ...func(*ExportLogQuery)) *UserQuery {
	query := (&ExportLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withNotificationSetting; query != nil {
		if err := _q.loadNotificationSetting(ctx, query, nodes, nil,
	...
	if query := _q.withTaskHistories; query != nil {
		if err := _q.loadTaskHistories(ctx, query, nodes,
			func(n *User) { n.Edges.TaskHistories = []*TaskHistory{} },
			func(n *User, e *TaskHistory) { n.Edges.TaskHistories = append(n.Edges.TaskHistories, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withExportLogs; query != nil {
		if err := _q.loadExportLogs(ctx, query, nodes,
			func(n *User) { n.Edges.ExportLogs = []*ExportLog{} },
			func(n *User, e *ExportLog) { n.Edges.ExportLogs = append(n.Edges.ExportLogs, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadNotificationSetting(ctx context.Context, query *NotificationSettingQuery, nodes []*User, init func(*User), assign func(*User, *NotificationSetting)) error {
...
func (_q *UserQuery) loadAssignedTasks(ctx context.Context, query *TaskQuery, nodes []*User, init func(*User), assign func(*User, *Task)) error {
	...
	for _, n := range neighbors {
		fk := n.AssignedTo
		if fk == nil {
			return fmt.Errorf(`foreign-key "assigned_to" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *UserQuery) loadTaskHistories(ctx context.Context, query *TaskHistoryQuery, nodes []*User, init func(*User), assign func(*User, *TaskHistory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(taskhistory.FieldChangedBy)
	}
	query.Where(predicate.TaskHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TaskHistoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "changed_by" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *UserQuery) loadExportLogs(ctx context.Context, query *ExportLogQuery, nodes []*User, init func(*User), assign func(*User, *ExportLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(exportlog.FieldUserID)
	}
	query.Where(predicate.ExportLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ExportLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
