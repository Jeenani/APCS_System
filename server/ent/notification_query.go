...
func (_q *NotificationQuery) QueryTask() *TaskQuery {
...
func (_q *NotificationQuery) QueryNotificationType() *NotificationTypeQuery {
...
func (_q *NotificationQuery) QueryPriority() *PriorityQuery {
...
func (_q *NotificationQuery) WithTask(opts ...func(*TaskQuery)) *NotificationQuery {
	query := (&TaskClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NotificationQuery) WithNotificationType(opts ...func(*NotificationTypeQuery)) *NotificationQuery {
	query := (&NotificationTypeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NotificationQuery) WithPriority(opts ...func(*PriorityQuery)) *NotificationQuery {
	query := (&PriorityClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	...
	if query := _q.withTask; query != nil {
		if err := _q.loadTask(ctx, query, nodes, nil,
			func(n *Notification, e *Task) { n.Edges.Task = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withPriority; query != nil {
		if err := _q.loadPriority(ctx, query, nodes, nil,
			func(n *Notification, e *Priority) { n.Edges.Priority = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NotificationQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Task)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Notification)
	for i := range nodes {
...
func (_q *NotificationQuery) loadNotificationType(ctx context.Context, query *NotificationTypeQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *NotificationType)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Notification)
	for i := range nodes {
...
func (_q *NotificationQuery) loadPriority(ctx context.Context, query *PriorityQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Priority)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Notification)
	for i := range nodes {
		if nodes[i].PriorityID == nil {
			continue
		}
...
func (_q *NotificationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withTask != nil {
			_spec.Node.AddColumnOnce(notification.FieldTaskID)
		}
		if _q.withNotificationType != nil {
			_spec.Node.AddColumnOnce(notification.FieldNotificationTypeID)
		}
		if _q.withPriority != nil {
			_spec.Node.AddColumnOnce(notification.FieldPriorityID)
		}
	}
