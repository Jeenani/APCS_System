...
type TaskHistoryQuery struct {
	...
	withTask       *TaskQuery
	withUser       *UserQuery
	withChangeType *ChangeTypeQuery
...
func (_q *TaskHistoryQuery) QueryUser() *UserQuery {
...
// QueryChangeType chains the current query on the "change_type" edge.
func (_q *TaskHistoryQuery) QueryChangeType() *ChangeTypeQuery {
	query := (&ChangeTypeClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TaskHistoryQuery) Clone() *TaskHistoryQuery {
	...
	return &TaskHistoryQuery{
		...
		withChangeType: _q.withChangeType.Clone(),
...
func (_q *TaskHistoryQuery) WithUser(opts ...func(*UserQuery)) *TaskHistoryQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithChangeType tells the query-builder to eager-load the nodes that are connected to
// the "change_type" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TaskHistoryQuery) WithChangeType(opts ...func(*ChangeTypeQuery)) *TaskHistoryQuery {
	query := (&ChangeTypeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withChangeType = query
	return _q
}
...
func (_q *TaskHistoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*TaskHistory, error) {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *TaskHistory, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withChangeType; query != nil {
		if err := _q.loadChangeType(ctx, query, nodes, nil,
			func(n *TaskHistory, e *ChangeType) { n.Edges.ChangeType = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TaskHistoryQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*TaskHistory, init func(*TaskHistory), assign func(*TaskHistory, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*TaskHistory)
	for i := range nodes {
		fk := nodes[i].ChangedBy
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "changed_by" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TaskHistoryQuery) loadChangeType(ctx context.Context, query *ChangeTypeQuery, nodes []*TaskHistory, init func(*TaskHistory), assign func(*TaskHistory, *ChangeType)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*TaskHistory)
	for i := range nodes {
		fk := nodes[i].ChangeTypeID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(changetype.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "change_type_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TaskHistoryQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUser != nil {
			_spec.Node.AddColumnOnce(taskhistory.FieldChangedBy)
		}
		if _q.withChangeType != nil {
			_spec.Node.AddColumnOnce(taskhistory.FieldChangeTypeID)
		}
	}
