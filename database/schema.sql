...
-- ============================================================
-- РАЗДЕЛ 8: НАЧАЛЬНЫЕ ДАННЫЕ (Seed Data)
-- ============================================================

INSERT INTO roles (name) VALUES
    ('admin'), ('chief_engineer'), ('asutp_chief'), ('engineer'), ('operator');

INSERT INTO priorities (name, color_hex, sort_order) VALUES
    ('high',   '#E53935', 1),
    ('medium', '#F9A825', 2),
    ('low',    '#2E7D32', 3);

INSERT INTO task_statuses (code, is_terminal) VALUES
    ('new',         FALSE),
    ('in_progress', FALSE),
    ('completed',   TRUE),
    ('cancelled',   TRUE);

INSERT INTO change_types (code) VALUES
    ('task_created'), ('title_changed'), ('description_changed'),
    ('due_date_changed'), ('priority_changed'), ('progress_changed'),
    ('status_changed'), ('assignee_changed');

INSERT INTO notification_types (code) VALUES
    ('reminder'), ('deadline'), ('update'), ('system');

INSERT INTO export_types (code) VALUES
    ('all'), ('completed'), ('selected');

INSERT INTO task_categories (name, icon_identifier, description) VALUES
    ('Датчики',              'icon_sensor',    'Датчики давления, температуры, расхода'),
...
INSERT INTO users (login, password_hash, full_name, initials, role_id) VALUES
    ('chief.engineer',
     '$2b$12$placeholder_bcrypt_hash_chief',
     'Сергей Волков', 'СВ',
     (SELECT id FROM roles WHERE name = 'chief_engineer')),
    ('asutp.chief',
     '$2b$12$placeholder_bcrypt_hash_asutp',
     'Иван Петров', 'ИП',
     (SELECT id FROM roles WHERE name = 'asutp_chief')),
    ('ivan.engineer',
     '$2b$12$placeholder_bcrypt_hash_ivan',
     'Алексей Сидоров', 'АС',
     (SELECT id FROM roles WHERE name = 'engineer')),
    ('operator1',
     '$2b$12$placeholder_bcrypt_hash_oper',
     'Мария Козлова', 'МК',
     (SELECT id FROM roles WHERE name = 'operator')),
    ('admin',
     '$2b$12$placeholder_bcrypt_hash_admin',
     'Администратор Системы', 'АС',
     (SELECT id FROM roles WHERE name = 'admin'));

INSERT INTO notification_settings (user_id)
SELECT id FROM users WHERE login IN ('chief.engineer', 'asutp.chief', 'ivan.engineer', 'operator1', 'admin');

INSERT INTO tasks (title, description, due_date, priority_id, status_id,
                   category_id, progress, created_by, assigned_to)
VALUES
    ('Проверка датчиков давления',
     ...
     (SELECT id FROM task_categories WHERE name = 'Датчики'),
     75,
    ...
    ('Обновление ПО контроллера',
     ...
     (SELECT id FROM task_statuses WHERE code = 'in_progress'),
     (SELECT id FROM task_categories WHERE name = 'Контроллеры'),
     40,
     (SELECT id FROM users WHERE login = 'ivan.petrov'),
     (SELECT id FROM users WHERE login = 'ivan.petrov')),
    ...
    ('Калибровка уровнемеров',
     ...
     (SELECT id FROM task_statuses WHERE code = 'in_progress'),
     (SELECT id FROM task_categories WHERE name = 'Уровнемеры'),
     20,
     (SELECT id FROM users WHERE login = 'ivan.petrov'),
     (SELECT id FROM users WHERE login = 'ivan.petrov')),
    ...
    ('Проверка шкафов управления',
     ...
     (SELECT id FROM task_categories WHERE name = 'Контроль оборудования'),
     0,
     (SELECT id FROM users WHERE login = 'ivan.petrov'),
     (SELECT id FROM users WHERE login = 'ivan.petrov'));
